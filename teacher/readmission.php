<?php
// filepath: /Applications/XAMPP/xamppfiles/htdocs/Untitled/habits365club/teacher/readmission.php
session_start();
if (!isset($_SESSION['teacher_username']) && !isset($_COOKIE['teacher_username'])) {
...
// Get teacher ID
$teacher_username = $_SESSION['teacher_username'] ?? $_COOKIE['teacher_username'];
$stmt = $db->prepare("SELECT id FROM users WHERE username = ? AND role = 'teacher'");
...
$batches = [];
$batchStmt = $db->prepare("SELECT b.id, b.name FROM batches b JOIN batch_teachers bt ON b.id = bt.batch_id WHERE bt.teacher_id = ?");
...
// Fetch all active students in teacher's batches
$query = "
...
<head>
    ...
    <title>Readmissions - Teacher</title>
